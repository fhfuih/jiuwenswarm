...
async def test_consume_workflow_events_converts_to_team_events_for_web(monkeypatch):
    """On a web channel _consume_workflow_events broadcasts native workflow.updated and team events."""
    broadcasted: list[dict[str, object]] = []
    event = {
        "event_type": "workflow.updated",
    ...
    assert broadcasted
    assert broadcasted[0] is event
    assert broadcasted[0]["event_type"] == "workflow.updated"
    team_events = [e for e in broadcasted if e.get("event_type") in ("team.member", "team.task")]
    assert team_events
