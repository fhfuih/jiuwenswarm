...
_WORKFLOW_CONTROL_STATE_KEY = "workflow_control"
...
def persist_workflow_control(session_id: str, controls: dict[str, dict]) -> None:
    """Persist editable SwarmFlow dispatch overlays, decoupled from run snapshots."""
    from jiuwenswarm.server.runtime.session.session_metadata import _read_metadata, _enqueue_write

    metadata = _read_metadata(session_id, cache_bust=True)
    if not metadata.get("session_id"):
        logger.warning(
            "[TeamHelpers] skipping workflow_control persist: failed to read "
            "session metadata (session_id=%s)",
            session_id,
        )
        return
    metadata[_WORKFLOW_CONTROL_STATE_KEY] = controls
    _enqueue_write(session_id, metadata)
    _write_workflow_control_sidecar(metadata, controls)


def restore_workflow_control(session_id: str) -> dict[str, dict] | None:
    """Restore editable workflow control overlays from session metadata."""
    from jiuwenswarm.server.runtime.session.session_metadata import _read_metadata

    metadata = _read_metadata(session_id, cache_bust=True)
    controls = metadata.get(_WORKFLOW_CONTROL_STATE_KEY)
    if not isinstance(controls, dict) or not controls:
        return None
    return {
        str(run_id): dict(spec)
        for run_id, spec in controls.items()
        if isinstance(spec, dict)
    }


def _write_workflow_control_sidecar(metadata: dict, controls: dict[str, dict]) -> None:
    """Best-effort copy next to the team SwarmFlow scripts for the next run()."""
    team_name = str(metadata.get("team_name") or "").strip()
    if not team_name:
        return
    try:
        sidecar = get_agent_teams_home() / team_name / "team-workspace" / "swarmflow" / "workflow-control.json"
        ...
        logger.warning("[TeamHelpers] workflow_control sidecar write failed: %s", exc)
...
#
# TUI 原生渲染 ``workflow.updated``。Web 同时订阅原生 ``workflow.updated``
#（工作流图）以及转换后的 ``team.member`` / ``team.task``（任务板）。当 web
# 端触发 swarmflow 时，每个 worker 仍转成 teammate 事件、每个 phase 仍转成
# task 事件，任务板与运行图共用同一份运行快照。
#
...
async def _consume_workflow_events(
    ...
    TUI keeps the native ``workflow.updated`` stream. Web also receives the
    native payload so the workflow graph panel can render phases / parallel
    agents; the same event is still translated into ``team.member`` /
    ``team.task`` so the existing task board keeps working.
    """
