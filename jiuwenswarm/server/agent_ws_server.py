...
class AgentWebSocketServer:
    ...
    async def _handle_command_workflows(self, ws: Any, request: AgentRequest, send_lock: asyncio.Lock) -> None:
        ...
        elif action in {"get_control", "set_control"}:
            from jiuwenswarm.server.runtime.agent_adapter.team_helpers import (
                persist_workflow_control,
                restore_workflow_control,
            )

            stored = restore_workflow_control(session_id) or {}
            if action == "get_control":
                ...
                if not isinstance(workflow_id, str) or not workflow_id.strip():
                    resp = AgentResponse(
                        ...
                        channel_id=channel_id,
                        ok=False,
                        payload={"error": "workflow_id is required for action=set_control"},
                    )
                    ...
                    resp = AgentResponse(
                        ...
                        channel_id=channel_id,
                        ok=False,
                        payload={"error": "control object is required for action=set_control"},
                    )
                else:
                    target_id = workflow_id.strip()
                    stored[target_id] = control
                    persist_workflow_control(session_id, stored)
                    resp = AgentResponse(
                        request_id=request.request_id,
                        channel_id=channel_id,
                        ok=True,
