...
function AppContent({
  ...
  const { loadProjects, setSelectedProject, setPendingDesignerGraphId } = useWorkspaceStore();
  ...
  const loadSessionMetadata = useCallback(async (targetSessionId: string): Promise<Session | null> => {
    try {
      ...
      applyWorkflowRunsToSession(targetSessionId, extractWorkflowRunsFromMetadata(session));
      extractWorkflowControlsFromMetadata(session).forEach((spec) => {
        useSessionStore.getState().setWorkflowControl(targetSessionId, spec);
      });
      return session;
    } catch (error) {
  ...
  const handleOpenDesignerGraph = useCallback(
    (graphId: string, projectId: string) => {
      const project = useWorkspaceStore.getState().projects.find((item) => item.project_id === projectId);
      if (project) setSelectedProject(project);
      setPendingDesignerGraphId(graphId);
      handleNavigate('designer');
    },
    [handleNavigate, setPendingDesignerGraphId, setSelectedProject],
  );
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <main className={`content ${activeNav === 'chat' ? 'content--chat' : ''} ${activeNav === 'designer' ? 'content--designer' : ''} ${isTeamAreaExpanded ? 'content--team-expanded' : ''}`}>
        {configError && (
          ...
          <>
            <div className="chat-layout flex-1 flex min-h-0 overflow-hidden">
              <ConversationSidebar
                ...
                onOpenDesigner={handleOpenDesignerGraph}
                isCronActive={false}
