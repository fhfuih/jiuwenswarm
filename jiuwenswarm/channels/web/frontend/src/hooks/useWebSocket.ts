...
import { extractWorkflowPayload, normalizeWorkflowRun } from '../features/workflowGraph/workflowGraphModel';
  ...
  useEffect(() => {
    const applyTeamMemberShutdown = (memberId: string, sessionId?: string) => {
      ...
      if (nextMembers.length === 0) {
        ...
        currentSessionStore.setWorkflowRuns(sessionId, []);
    ...
    const unsubs = [
      ...
      webClient.on('workflow.updated', ({ payload }) => {
        const sessionId = resolveEventSessionId(payload);
        if (!sessionId) return;
        const run = normalizeWorkflowRun(extractWorkflowPayload(payload));
        if (!run) return;
        useSessionStore.getState().upsertWorkflowRun(sessionId, run);
      }),
