...
export function supportsNodeToolbar(nodeType: string): boolean {
  return isMediaNodeType(nodeType) || nodeType === 'text' || nodeType === 'table';
}
...
export function readMediaConfig(
  ...
  const topPrompt = typeof (config as { prompt?: unknown } | undefined)?.prompt === 'string'
  ...
  return {
    ...
    generate: {
      prompt: raw.generate?.prompt || topPrompt || '',
  ...
  return {
    ...
    interaction_mode:
      current.interaction_mode === 'upload' || current.interaction_mode === 'edit'
        ...
        : 'generate',
    ...(typeof nextGenerate.prompt === 'string' ? { prompt: nextGenerate.prompt } : {}),
