...
function DesignerCanvasInner({ graph }: DesignerCanvasProps) {
  ...
  const fitPadding = useMemo(
    () => ({
      top: 0.08,
      bottom: 0.1,
      left: chatCollapsed ? 0.06 : 0.28,
      right: materialsCollapsed ? 0.06 : 0.26,
    }),
    [chatCollapsed, materialsCollapsed],
  );
  ...
  }, [reactFlowGraph, setNodes, setEdges]);

  useEffect(() => {
    requestAnimationFrame(() => {
      void fitView({ padding: fitPadding, duration: 200 });
    });
  }, [fitPadding, fitView, graph.graph_id]);
  ...
  return (
    ...
    >
      ...
      <Controls position="top-right" showFitView className="designer-canvas__controls" />
