...
export function DesignerNodeToolbar({ nodeId, nodeType }: DesignerNodeToolbarProps) {
  ...
  const media = readMediaConfig(config);
    ...
    >
      <div className="designer-node-toolbar__tabs" role="tablist" aria-label={t('designer.toolbar.modeLabel')}>
        <button
          ...
          className={`designer-node-toolbar__tab${mode === 'upload' ? ' is-active' : ''}`}
          data-testid="designer-node-toolbar-tab-upload"
          onClick={() => setMode('upload')}
        >
          {t('designer.toolbar.upload')}
        </button>
        <button
          type="button"
          role="tab"
