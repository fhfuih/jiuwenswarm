...
type DesignerStore = {
  ...
  setMaterialsCollapsed: (collapsed: boolean) => void;
  ...
  setNodeOutputRef: (nodeId: string, outputRef: DesignerExecutionGraph['nodes'][number]['output_ref']) => void;
  updateNodeConfig: (
    nodeId: string,
...
export const useDesignerStore = create<DesignerStore>((set, get) => ({
  ...
  setMaterialsCollapsed: (collapsed) => set({ materialsCollapsed: collapsed }),
  ...
  setNodeOutputRef: (nodeId, outputRef) => {
    ...
    const nodes = graph.nodes.map((node) =>
      node.id === nodeId ? { ...node, output_ref: outputRef } : node,
    );
    set({
      domainGraph: {
        ...graph,
