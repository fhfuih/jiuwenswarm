...
> 💡 **提示**：默认模型是系统运行的基础，必须正确配置。视频理解、音频、视觉、图片生成、视频生成模型为可选配置，当您需要相应多模态能力时再配置即可。视频理解 / 音频 / 视觉 / 图片生成 / 视频生成在 Web「设置 → 智能体 → 工具」中配置。图片生成也可通过主配置（`models.image_gen`）或环境变量（`IMAGE_GEN_API_BASE` 等）配置。
...
| `model_provider` | `client_provider`     | 模型协议/供应商类型，决定请求格式与鉴权方式    | 支持 `OpenAI`、`DeepSeek`、`DashScope`、`SiliconFlow`、`InferenceAffinity`、`OpenRouter`、`OpenAIAccount`，用于适配不同服务商的 API 格式；视频理解/音频/视觉模型当前仅支持 `OpenAI`，视频生成模型当前默认 `DashScope` |
...
- **默认值**：`false`（关闭）
- **作用**：启用后，Team 模式下 Leader 可调用 `swarmflow()`；Web 右侧工具面板出现 **工作流** 页签（阶段从上到下、同阶段 Agent 并排表示并行），TUI 用 `/swarmflows` 查看运行树。
