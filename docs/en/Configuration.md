...
> 💡 **Tip**: The default model is essential for system operation and must be configured correctly. Video understanding, audio, vision, image generation, and video generation models are optional; configure them only when multimodal capabilities are needed. Video understanding / audio / vision / image generation / video generation are configured in Web **Settings → Agent → Tools**. Image generation can also be configured via the main config (`models.image_gen`) or environment variables (`IMAGE_GEN_API_BASE`, etc.).
...
| `model_provider` | `client_provider`           | Model protocol/provider type that determines request format and auth | Supports `OpenAI`, `DeepSeek`, `DashScope`, `SiliconFlow`, `InferenceAffinity`, `OpenRouter`, `OpenAIAccount` for API format adaptation; video understanding/audio/vision models currently support `OpenAI` only, and the video generation model defaults to `DashScope` |
